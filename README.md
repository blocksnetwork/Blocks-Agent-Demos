...
| [`design_blocks`](./design_blocks) | Claude Code / Cursor send a one-line brief and get back three rendered design comps built from decomposed reference designs, with credited bank photography as hero imagery — winner expanded into theme.css, hero.png, og.png | Blocks provider agent, TypeScript, vLLM + Qwen3.5-4B (Claude optional), CLIP retrieval + scoring, satori/resvg |
...
typography, competing palettes, hero imagery cut from licensed bank photography
and gradient-mapped to the exact token hexes — scored with CLIP against a reference
...
By default nothing here calls a hosted API. The agents run open-weights models on
hardware you control, under licences that permit commercial use, and they share a
single GPU between them. The one exception is opt-in: give `design_blocks` an
`ANTHROPIC_API_KEY` and it authors its composition specs with Claude, keeping the
local model as the fallback.
