...
export const FAILURE_MESSAGES: Record<FailureKind, Message> = {
  config: {
    ...
    body: "The server could not authenticate with Blocks, so the task was never sent. Put the agent's key in hook-web/.env.local as BLOCKS_API_KEY and restart the dev server. Nothing about your recording is wrong.",
    primary: "Try again",
  },
  offline: {
    title: "The models are not answering.",
    body: "Hook Finder runs transcription and ranking on one GPU box. Right now that box is not picking tasks up, so there is nothing to send this to. Your recording never left this tab.",
    primary: "Try again",
  },
  network: {
