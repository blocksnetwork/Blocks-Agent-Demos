...
import { streamHooks } from "@/lib/stream";
...
export default function Home() {
  ...
  const runJob = useCallback(
    async (blob: Blob, filename: string) => {
      ...
      try {
        await streamHooks(blob, filename, controller.signal, (event) => {
          switch (event.type) {
            case "accepted":
      ...
      if (!looksLikeMedia(file)) {
        setScreen({
          ...
          message: preparationMessage(
            `${file.name} is not audio or video. Hook Finder reads what was said, so it needs a recording.`,
          ),
          retry: "reset",
        });
        return;
