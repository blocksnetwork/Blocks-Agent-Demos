...
The consumer side of [`hook_finder_blocks`](..). Drop a recording, watch the run,
...
vLLM on `:8000`. See the [agent's README](../../README.md#hook_finder_blocks) for
the deploy.
...
`TaskClient` is constructed in exactly one place, `src/app/api/hooks/route.ts`,
which is a Node runtime route handler. The prepared audio is posted to it, and
the task's progress is streamed back to the page as server-sent events, so the
three-step panel follows the handler's real status updates rather than a timer.
